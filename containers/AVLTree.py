...
class AVLTree(BST):
    ...
    def _left_rotate(node):
        ...
        if node is None or node.right is None:
            return node
        r1 = Node(node.right.value)
        r1.right = node.right.right
        r1.left = Node(node.value)
        r1.left.left = node.left
        r1.left.right = node.right.left
        return r1
    ...
    def _right_rotate(node):
        ...
        if node is None or node.left is None:
            return node
        r1 = Node(node.left.value)
        r1.left = node.left.left
        r1.right = Node(node.value)
        r1.right.left = node.left.right
        r1.right.right = node.right
        return r1
    ...
    def rebalance(self, node):
        if node is None:
            return
        elif self._balance_factor(node) in [-2, 2]:
